...
class Sample(Base, AutoBaseMixin):
    ...
    field_sample_id: Mapped[str] = mapped_column(
        unique=True,
        comment="User-defined ID for field tracking.")
    ...
    sample_top: Mapped[Optional[float]] = mapped_column(
        Float,
        comment="Top depth of a discrete sample interval.")
    sample_bottom: Mapped[Optional[float]] = mapped_column(
        Float,
        comment="Bottom depth of a discrete sample interval.")
    ...
    __table_args__ = (
        UniqueConstraint(
            'thing_id', 'sample_date', 'sample_matrix', 'sample_top',
            'sample_bottom', 'duplicate_sample_number', 'field_sample_id', 'qc_sample',
