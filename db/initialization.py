...
def _parse_app_read_members() -> list[str]:
    ...
    raw_members = members.split(",")
    parsed = [member.strip() for member in raw_members if member.strip()]
...
def grant_app_read_members(executor: Session | Connection | None) -> None:
    ...
    for member in members:
        ...
        stmt = text(
            ...
            BEGIN
                IF EXISTS (
                    SELECT 1 FROM pg_roles WHERE rolname = '{safe_member}'
                ) THEN
...
def sync_search_vector_triggers(session: Session) -> None:
    """Ensure searchable triggers exist for every TSVector column."""
