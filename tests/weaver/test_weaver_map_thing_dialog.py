...
def test_weaver_spring_by_id():
    response = client.get("/thing/1?format=geojson") #TODO: Same question as above, id would be a spring id
    assert response.status_code == 200
    data = response.json()
    assert "type" in data
    assert "properties" in data
...
def test_weaver_water_chemistry_by_well_id():
    response = client.get("/observation/water-chemistry?thing_id=1") #TODO: Not sure this is how we would want to do this?
    assert response.status_code == 200
    data = response.json()
