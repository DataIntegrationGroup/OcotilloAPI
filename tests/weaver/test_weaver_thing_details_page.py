...
def test_weaver_thing_equipment_by_id():
    ...
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, dict) # just assume data dictionary for now used by Weaver (previously Equipment)
...
def test_weaver_water_chemistry_by_well_id():
    response = client.get("/observation/water-chemistry?thing_id=1") # again not sure this is the right endpoint
    assert response.status_code == 200
    data = response.json()
