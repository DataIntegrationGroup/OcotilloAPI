...
import pytest
from db import Location, LocationThingAssociation, MeasuringPointHistory, Thing
...
def shared_well():
    ...
    with session_ctx() as session:
        thing = _create_well_with_defaults(session, f"Shared-Well-{uuid4().hex[:8]}")
        thing_id = thing.id
...
def test_thing_chemistry_sample_infos_empty_by_default():
    ...
    with session_ctx() as session:
        new_thing = _create_well_with_defaults(session, f"Empty-Test-{uuid4().hex[:8]}")
...
def test_cascade_delete_thing_deletes_sample_infos():
    ...
    with session_ctx() as session:
        test_thing = _create_well_with_defaults(
            session, f"Cascade-Test-{uuid4().hex[:8]}"
        )
...
def test_multiple_sample_infos_per_thing():
    ...
    with session_ctx() as session:
        test_thing = _create_well_with_defaults(
            session, f"Multi-SI-Test-{uuid4().hex[:8]}"
        )
...
def _create_well_with_defaults(session, name: str) -> Thing:
