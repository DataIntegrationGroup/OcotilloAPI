...
# ===============================================================================
"""
Backfill legacy NMA chemistry rows into the Ocotillo Observation schema.

Supports NMA_Radionuclides and NMA_MinorTraceChemistry.  Each backfill
reads legacy rows joined with NMA_Chemistry_SampleInfo, resolves each
...
# ---------------------------------------------------------------------------
# Minor Trace Chemistry backfill
# ---------------------------------------------------------------------------
def _backfill_minor_trace_chemistry_impl(session: Session) -> BackfillResult:
    result = BackfillResult()
    ...
    legacy_rows = session.execute(
        select(NMA_MinorTraceChemistry, NMA_Chemistry_SampleInfo.nma_sample_pt_id)
        .join(
            NMA_Chemistry_SampleInfo,
            NMA_MinorTraceChemistry.chemistry_sample_info_id
            == NMA_Chemistry_SampleInfo.id,
        )
        .order_by(NMA_MinorTraceChemistry.id)
    ).all()

    logger.info("Processing %d legacy MinorTraceChemistry rows", len(legacy_rows))
    ...
    for row, nma_sample_pt_id in legacy_rows:
        ...
        if sample_pt_key is None or sample_pt_key not in sample_cache:
            result.skipped_orphans += 1
            logger.debug(
                "Skipping MinorTraceChemistry GlobalID=%s — no matching Sample for SamplePtID=%s",
                global_id_str,
                sample_pt_key,
            )
            continue
    ...
    session.commit()
    logger.info(
        "MinorTraceChemistry backfill complete: inserted=%d updated=%d skipped_orphans=%d errors=%d",
        result.inserted,
        result.updated,
        result.skipped_orphans,
        len(result.errors),
...
def backfill_minor_trace_chemistry() -> BackfillResult:
    """Top-level runner for the minor trace chemistry backfill."""
    with session_ctx() as session:
        return _backfill_minor_trace_chemistry_impl(session)
