...
# ===============================================================================
"""split field operations alternate ids into per-organization columns

Rebuilds ogc_internal_water_well_field_operations so each organization's
alternate identifier gets its own column, replacing the single alternate_ids
column that packed them all into one string as "org: id" pairs.

That string could not be used as a join key, which is the only thing an
alternate identifier is for. A crew matching this layer against an NMOSE
extract had to parse a delimited cell; ArcGIS Pro could only filter it with a
LIKE. One column per organization gives each identifier a real key.

Ten columns, because organization alone does not identify the identifier.
NMOSE issues three different ones and the relation column is what tells them
apart: an OSEPOD point-of-diversion number and an OSEWellTagID are not
interchangeable, so they do not share a column. The other six organizations use
a single relation each.

alternate_id_other is the catch-all. The seven organizations are what
thing_id_link holds today, and the column is not restricted to them, so a
fixed list of columns cannot stay complete. Without the catch-all the first
identifier from a new organization would drop silently. It keeps the
"org: id" form because it has to name the organization.

Blank identifiers are filtered out rather than aggregated. Roughly 150 NMOSE
same_as links carry an empty alternate_id, and letting them through would
produce a leading comma or a cell that looks populated but is not.

Aggregated once with FILTER rather than per column with correlated subqueries:
ten correlated subqueries would be ten index lookups per row on a layer that
already takes 16 s to download in full, where a single grouped scan of
thing_id_link is a few hundred milliseconds regardless of how many columns come
out of it.

Per-organization is still multi-valued -- over a thousand wells carry two PLSS
ids -- so every column stays comma-joined text, matching the rest of the layer.

Only the feature view changes. The _stats matview carries no alternate ids, so
...
# (published column, alternate_organization, extra relation predicate).
# These seven organizations are every value thing_id_link holds today. The
# column is not restricted to them, so anything else lands in
# alternate_id_other.
#
# The relation predicate is only set for NMOSE, which issues three different
# identifiers under one organization name and needs relation to tell them
# apart. Every other organization here uses a single relation.
ALTERNATE_ID_COLUMNS = (
    ("alternate_id_nmbgmr", "NMBGMR", None),
    ("alternate_id_nmose_pod", "NMOSE", "tl.relation = 'OSEPOD'"),
    ("alternate_id_nmose_well_tag", "NMOSE", "tl.relation = 'OSEWellTagID'"),
    (
        "alternate_id_nmose",
        "NMOSE",
        "tl.relation NOT IN ('OSEPOD', 'OSEWellTagID')",
    ),
    ("alternate_id_plss", "PLSS", None),
    ("alternate_id_usgs", "USGS", None),
    ("alternate_id_nmed", "NMED", None),
    ("alternate_id_twdb", "TWDB", None),
    ("alternate_id_unknown", "Unknown", None),
)

# What downgrade restores: the single column this revision replaces, verbatim
# from f2a3b4c5d6e7.
COMBINED_ALTERNATE_IDS = """(
                SELECT string_agg(
                    DISTINCT
...
def _split_columns_sql() -> str:
    """The ten per-organization columns, as they appear in the select list."""
    return (
        ",\n            ".join(f"ail.{name}" for name, _, _ in ALTERNATE_ID_COLUMNS)
        + ",\n            ail.alternate_id_other"
    )
...
def _predicate(organization: str, relation: str | None) -> str:
    organization_test = f"tl.alternate_organization = {_quote_literal(organization)}"
    if relation is None:
        return organization_test
    return f"{organization_test} AND {relation}"


def _alternate_id_cte() -> str:
    """One grouped pass over thing_id_link, one FILTER per published column.

    string_agg because a well can hold more than one identifier from the same
    organization; the ORDER BY keeps the joined value stable between runs so a
    diff of two exports shows real changes only.
    """
    columns = ",\n    ".join(
        "string_agg(DISTINCT tl.alternate_id, ', ' ORDER BY tl.alternate_id)\n"
        f"        FILTER (WHERE {_predicate(organization, relation)})\n"
        f"        AS {_safe_relation_name(name)}"
        for name, organization, relation in ALTERNATE_ID_COLUMNS
    )
    # Derived, not restated: the catch-all has to exclude exactly the
    # organizations that have a column above, or an identifier is published
    # twice or not at all.
    named = ", ".join(
        f"'{organization}'"
        for organization in sorted({org for _, org, _ in ALTERNATE_ID_COLUMNS})
    )
    return f"""SELECT
    tl.thing_id,
    {columns},
    string_agg(
        DISTINCT tl.alternate_organization || ': ' || tl.alternate_id,
        ', '
        ORDER BY tl.alternate_organization || ': ' || tl.alternate_id
    )
        FILTER (
            WHERE tl.alternate_organization IS NULL
            OR tl.alternate_organization NOT IN ({named})
        )
        AS alternate_id_other
FROM thing_id_link AS tl
WHERE NULLIF(btrim(tl.alternate_id), '') IS NOT NULL
GROUP BY tl.thing_id"""
...
ALTERNATE_ID_CTE = _alternate_id_cte()


def _create_feature_view(alternate_id_columns: str, *, split: bool) -> str:
    """Build the view. ``split`` selects the per-organization columns.

    When false the aggregate CTE and its join are left out entirely, so
    downgrade restores f2a3b4c5d6e7's view rather than a copy of it carrying an
    unused CTE.
    """
    # Copied rather than imported: a revision has to reproduce the SQL it ran
    # with, and CREATE OR REPLACE VIEW cannot change a column list, so the view
    # is dropped and rebuilt either way.
    alternate_id_cte_block = (
        f"\n        alternate_id_links AS (\n{ALTERNATE_ID_CTE}\n        ),"
        if split
        else ""
    )
    alternate_id_join = (
        "\n        LEFT JOIN alternate_id_links AS ail ON ail.thing_id = t.id"
        if split
        else ""
    )
...
{LATEST_LOCATION_CTE}
        ...
        LEFT JOIN screens AS scr ON scr.thing_id = t.id{alternate_id_join}
        LEFT JOIN LATERAL (
...
def _rebuild(alternate_id_columns: str, *, split: bool) -> None:
    _check_required_tables()
    safe_feature = _safe_relation_name(FEATURE_VIEW)
    op.execute(text(f"DROP VIEW IF EXISTS {safe_feature}"))
    op.execute(text(_create_feature_view(alternate_id_columns, split=split)))


def upgrade() -> None:
    _rebuild(_split_columns_sql(), split=True)


def downgrade() -> None:
    _rebuild(COMBINED_ALTERNATE_IDS, split=False)
