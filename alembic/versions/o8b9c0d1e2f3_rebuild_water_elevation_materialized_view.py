"""rebuild water elevation materialized view
...
def _create_water_elevation_view_pre_feet_fix() -> str:
    return f"""
        CREATE MATERIALIZED VIEW ogc_water_elevation_wells AS
        WITH latest_location AS (
{LATEST_LOCATION_CTE}
...
def _required_tables_present() -> None:
    bind = op.get_bind()
...
def _rebuild(create_sql: str, comment: str) -> None:
    op.execute(text("DROP MATERIALIZED VIEW IF EXISTS ogc_water_elevation_wells"))
    op.execute(text(create_sql))
    op.execute(
        text(
            "COMMENT ON MATERIALIZED VIEW ogc_water_elevation_wells IS " f"'{comment}'"
        )
    )
    op.execute(
        text(
...
def upgrade() -> None:
    _required_tables_present()
    _rebuild(
        _create_water_elevation_view(),
...
def downgrade() -> None:
    _rebuild(
        _create_water_elevation_view_pre_feet_fix(),
