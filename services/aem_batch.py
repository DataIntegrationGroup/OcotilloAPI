...
def build_acquisition_file_list(
    df: pd.DataFrame,
    survey_id: str,
    resolve_source_path,
) -> list[dict]:
    """Build local raw acquisition CSV paths for timestamp enrichment."""
    raw_mask = (
        (df["survey_id"] == survey_id)
        & (df["detected_type"] == "geotech_raw_csv")
        & (df["action"] == "MOVE")
...
def run_batch(
    ...
    skip_soundings_upload: bool = True,
    ...
    Args:
        ...
        skip_soundings_upload: Skip loading parsed soundings into PostGIS.
            Defaults to True; callers must explicitly opt in to uploads.
    ...
    failed = 0
    errors = []
    debug_stac_remaining_by_collection = {} if debug_stac_limit else None
    ...
    for idx, (_, row) in enumerate(ingest_df.iterrows()):
        ...
        try:
            ...
            if detected_type in INVERSION_INGESTIBLE_TYPES:
                ...
                acquisition_files = build_acquisition_file_list(
                    migration_runner.df,
                    survey_id,
                    migration_runner.resolve_source_path,
                )
                ...
                ingest_result = run_ingest(
                    ...
                    acquisition_file_paths=acquisition_files,
                    raw_manifest_notes=(
                    ...
                    debug_stac_remaining_by_collection=debug_stac_remaining_by_collection,
                )
