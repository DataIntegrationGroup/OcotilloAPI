...
those owners in ``PROJECT_AREA_MAPPINGS`` is exactly the layer-18 membership.
...
2. Re-points every layer-18 owner group's ``parent_group_id`` at that container.
...
# The distinct groups that own a layer-18 boundary. Sorted for stable output.
LAYER18_OWNER_NAMES: tuple[str, ...] = tuple(
    sorted({mapping.group_name for mapping in PROJECT_AREA_MAPPINGS.values()})
)
